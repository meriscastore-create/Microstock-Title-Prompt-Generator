...
const KeywordCheckerIcon: React.FC<{ className?: string }> = ({ className = "w-4 h-4" }) => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={className}>
    <path strokeLinecap="round" strokeLinejoin="round" d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z" />
  </svg>
);

const MagicWandIcon: React.FC<{ className?: string }> = ({ className = "w-4 h-4" }) => (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={className}>
...
const LightbulbIcon: React.FC<{ className?: string }> = ({ className = "w-4 h-4" }) => (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={className}>
...
const SlidersIcon: React.FC<{ className?: string }> = ({ className = "w-4 h-4" }) => (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={className}>
...
const Toast: React.FC<{ message: string; type: 'success' | 'error'; onClose: () => void }> = ({ message, type, onClose }) => {
  ...
  return (
    <div className={`flex items-center justify-between p-3 rounded-xl shadow-lg border backdrop-blur-md ${bgClass} animate-fade-in mb-3 text-sm`}>
...
const ApiKeyModal: React.FC<ApiKeyModalProps> = ({ isOpen, onClose, onSave }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 animate-fade-in p-4">
            <div className="glass-panel rounded-2xl p-6 max-w-md w-full transform transition-all shadow-2xl">
                <h2 className="text-xl font-bold mb-3 text-white">Gemini API Key</h2>
                ...
                <input
                    ...
                    className="w-full glass-input rounded-xl px-3 py-2 mb-5 text-white placeholder-gray-400 transition-all text-sm"
                />
                <div className="flex justify-end gap-2">
                    <button onClick={onClose} className="px-3 py-1.5 rounded-xl text-gray-300 hover:text-white hover:bg-white/5 transition-colors font-medium text-xs">
                        Cancel
                    </button>
                    <button onClick={handleSave} className="bg-gradient-to-r from-brand-primary to-brand-secondary hover:from-brand-primary/90 hover:to-brand-secondary/90 text-white font-bold py-1.5 px-4 rounded-xl shadow-lg shadow-brand-primary/25 transition-all transform hover:scale-[1.02] text-xs">
                        Save Key
                    </button>
                </div>
            </div>
...
const StylePreferencesPopover: React.FC<StylePreferencesPopoverProps> = ({ isOpen, onClose, preferences, selectedPreferences, onToggle, anchorRef }) => {
    ...
    return (
        <div ref={popoverRef} className="absolute z-20 mt-2 w-64 sm:w-80 bg-slate-900 border border-white/10 rounded-2xl shadow-2xl p-3 right-0 animate-fade-in">
            <div className="flex justify-between items-center mb-3 border-b border-white/10 pb-2">
                <h3 className="font-bold text-sm text-white">Style Preferences</h3>
            ...
            <div className="grid grid-cols-2 gap-1.5 max-h-56 overflow-y-auto pr-1 custom-scrollbar">
                {preferences.map(pref => (
                    <button
                        ...
                        className={`text-[10px] sm:text-xs text-left px-2 py-1.5 rounded-lg transition-all duration-200 ${
...
const App: React.FC = () => {
    ...
    return (
        <div className="min-h-screen p-3 sm:p-5 lg:p-6 relative overflow-hidden">
            ...
            <header className="text-center mb-8 relative z-10">
                <button
                    ...
                    className="absolute top-0 right-0 bg-white/10 hover:bg-white/20 text-white p-1.5 rounded-full backdrop-blur-md transition-all border border-white/10 shadow-lg"
                    aria-label="Set API Key"
                >
                    <KeyIcon />
                </button>
                <div className="inline-block mb-2">
                    <span className="px-2.5 py-0.5 rounded-full text-[10px] font-medium bg-gradient-to-r from-brand-primary/20 to-brand-secondary/20 border border-brand-primary/30 text-indigo-200 uppercase tracking-wider">
                        AI-Powered Generator
                    </span>
                </div>
                <h1 className="text-3xl sm:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-indigo-300 via-purple-300 to-pink-300 tracking-tight drop-shadow-lg">
                    Microstock Studio
                </h1>
                <p className="mt-2 text-base text-gray-300 max-w-2xl mx-auto leading-relaxed font-light">
                    Create SEO-optimized titles and detailed pattern prompts with a single keyword.
                </p>
            </header>

            <main className="max-w-7xl mx-auto relative z-10">
                <div className="glass-panel rounded-2xl p-4 sm:p-6 mb-8 max-w-3xl mx-auto transform transition-all hover:shadow-brand-primary/10">
                        ...
                        <div className="relative flex-grow w-full group">
                             <input
                                ...
                                placeholder={
                                    ...
                                    : "Enter a keyword (e.g., Christmas Tree)"
                                }
                                disabled={isCombining}
                                className="w-full glass-input rounded-xl px-3 py-2.5 text-sm text-white placeholder-gray-400 transition-all shadow-inner pr-10 disabled:opacity-60"
                            />
                            {(isCombining) ? (
                        ...
                        <div className="flex w-full sm:w-auto gap-2">
                             {userInput.trim().length > 0 ? (
                                <button
                                    ...
                                    className="flex-grow sm:flex-grow-0 flex items-center justify-center bg-gray-800/50 hover:bg-gray-700/50 border border-white/10 text-white font-semibold py-2 px-3 rounded-xl transition duration-200 disabled:opacity-50 disabled:cursor-not-allowed backdrop-blur-md shadow-lg text-xs"
                                    title="Combine with a new unique keyword"
                                >
                                    {isCombining ? <Spinner /> : <MagicWandIcon />}
                                    <span className="hidden sm:inline sm:ml-1.5">Mix</span>
                                </button>
                             ) : (
                                <button
                                ...
                                >
                                    ...
                                    <span className="hidden sm:inline sm:ml-1.5">Ideas</span>
                                </button>
                             )}
                            <button
                                onClick={handleGenerate}
                                disabled={isLoading || isSuggestingTopics || isCombining || !userInput.trim()}
                                className="flex-grow sm:flex-grow-0 flex items-center justify-center bg-gradient-to-r from-brand-primary to-brand-secondary hover:from-indigo-500 hover:to-purple-500 text-white font-bold py-2 px-5 rounded-xl transition-all duration-200 shadow-lg shadow-brand-primary/30 transform active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed disabled:shadow-none text-xs"
                            >
                                {isLoading ? (
                                    <>
                    ...
                    {/* SUGGESTION AREA */}
                    <div className={`transition-all duration-500 ease-in-out ${showSuggestionArea ? 'max-h-[1200px] opacity-100 mt-6' : 'max-h-0 opacity-0 mt-0'} overflow-hidden`}>
                        {isSuggestingTopics ? (
                            <div className="flex justify-center items-center py-8">
                                <div className="flex flex-col items-center gap-3">
                                    <div className="relative">
                                        ...
                                        <div className="absolute top-0 left-0 w-8 h-8 border-2 border-transparent border-b-brand-secondary/50 rounded-full animate-spin animation-delay-1000"></div>
                                    </div>
                                    <p className="text-gray-400 font-light animate-pulse text-xs">Analyzing market trends...</p>
                                </div>
                            </div>
                        ) : activeTopic ? (
                                ...
                                <div className="flex items-center justify-between mb-4">
                                    <div className="flex items-center gap-3">
                                        <button 
                                            ...
                                            className="flex items-center gap-1.5 text-gray-300 hover:text-white transition-colors bg-white/5 hover:bg-white/10 px-2.5 py-1 rounded-lg border border-white/5 text-xs"
                                        >
                                ...
                                {isSuggestingSpecifics ? (
                                    <div className="flex justify-center items-center py-8">
                                         <div className="flex flex-col items-center gap-2">
                                            <Spinner className="h-6 w-6 text-brand-secondary" />
                                            <p className="text-gray-400 text-xs">Finding high-demand keywords...</p>
                                        </div>
                                    </div>
                                ) : (
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2">
                                        {specificKeywords.map((kw, idx) => (
                                            <button
                                                ...
                                                className="text-left p-2.5 bg-white/5 hover:bg-white/10 border border-white/5 hover:border-brand-primary/50 rounded-xl transition-all flex justify-between items-start group hover:-translate-y-1 hover:shadow-lg"
                                            >
                                                <span className="text-xs text-gray-200 group-hover:text-white pr-2 font-medium">{kw.name}</span>
                                                <span className={`text-[10px] font-mono font-bold px-1.5 py-0.5 rounded bg-black/30 ${kw.score > 85 ? 'text-green-400' : 'text-yellow-400'}`}>
                                                    {kw.score}
                                                </span>
                                            </button>
                                        ))}
                            ...
                            <div className="flex flex-col gap-4">
                                <div className="flex justify-end">
                                    <button 
                                        ...
                                        className="flex items-center gap-1.5 text-[10px] font-medium text-gray-400 hover:text-brand-primary transition-colors bg-black/20 px-2 py-1 rounded-lg border border-white/5 hover:border-brand-primary/30"
                                    >
                                        <RefreshIcon className="w-3 h-3" /> Refresh Trends
                                    </button>
                                </div>
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                                    {topicCategories.map((cat, idx) => (
                                        <div key={idx} className="flex flex-col h-full bg-white/5 rounded-2xl p-4 border border-white/5 hover:border-white/10 transition-all">
                                            <h3 className="font-bold text-sm mb-3 text-center text-transparent bg-clip-text bg-gradient-to-br from-white to-gray-400 min-h-[2.5rem] flex items-center justify-center border-b border-white/5 pb-2">
                                                {cat.category}
                                            </h3>
                                            <div className={`flex flex-col gap-1 overflow-y-auto pr-1 custom-scrollbar ${cat.topics.length > 12 ? 'max-h-[300px]' : 'h-auto'}`}>
                                                {cat.topics.map((topic, tIdx) => (
                                                    <button
                                                        key={tIdx}
                                                        onClick={() => handleTopicClick(topic, cat.category)}
                                                        className="text-left px-2 py-1.5 rounded-lg text-xs text-gray-400 hover:text-white hover:bg-brand-primary/20 transition-all duration-150"
                                                    >
                                                        {topic}
                                                    </button>
                                                ))}
                ...
                <div className={`grid ${gridLayoutClass} gap-6 transition-all duration-300`}>
                    <div className="flex flex-col gap-6">
                        {generatedTitle && (
                            <div className={`glass-panel rounded-2xl p-5 sm:p-6 animate-fade-in shadow-xl ${isPreferencesOpen ? 'z-30 relative' : ''}`}>
                                <div className="flex justify-between items-center mb-4">
                                    <h2 className="text-xl font-bold text-white">Generated Title</h2>
                                    <span className={`text-[10px] font-mono px-1.5 py-0.5 rounded bg-black/30 ${titleCharCountColor}`}>{titleCharCount} / 170</span>
                                </div>
                                ...
                                <div className="relative group">
                                    ...
                                    <div className="relative bg-black/40 rounded-xl p-4 border border-white/10">
                                        <p className="text-base text-gray-200 leading-relaxed">{generatedTitle}</p>
                                    </div>
                                </div>
                                ...
                                <div className="flex flex-col sm:flex-row gap-2 mt-5">
                                    <button
                                        ...
                                        className="flex-1 flex items-center gap-1.5 bg-white/5 hover:bg-white/10 border border-white/10 text-gray-200 font-medium py-1 px-2 rounded-xl transition duration-200 justify-center disabled:opacity-50 text-[11px]"
                                    >
                                        {isRefreshingTitle ? <Spinner className="h-3 w-3" /> : <RefreshIcon className="w-3 h-3" />} Elements
                                    </button>
                                    ...
                                    <div className="relative flex w-full flex-1">
                                        <button
                                            ...
                                            disabled={isCreatingPrompt || isRefreshingTitle}
                                            className="flex-grow flex items-center gap-1.5 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-semibold py-1 px-2 rounded-l-xl transition duration-200 justify-center disabled:opacity-50 shadow-lg shadow-purple-900/20 text-[11px]"
                                        >
                                            {isCreatingPrompt ? <Spinner className="h-3 w-3" /> : <MagicWandIcon className="w-3 h-3" />}
                                            JSON Prompt
                                        </button>
                                        <button
                                            ref={preferencesButtonRef}
                                            onClick={() => setIsPreferencesOpen(prev => !prev)}
                                            className="flex items-center justify-center bg-indigo-700 hover:bg-indigo-600 border-l border-indigo-800 text-white font-bold py-1 px-2 rounded-r-xl transition duration-200 relative"
                                        >
                                            <SlidersIcon className="w-3 h-3" />
                                            {selectedStylePreferences.length > 0 && (
                                                <span className="absolute -top-1.5 -right-1.5 bg-red-500 text-white text-[9px] font-bold rounded-full h-4 w-4 flex items-center justify-center shadow-sm border border-red-400">
                                    ...
                                    <button onClick={() => handleCopy(generatedTitle, 'title')} className="flex-1 flex items-center gap-1.5 bg-blue-600/80 hover:bg-blue-500/80 text-white font-semibold py-1 px-2 rounded-xl transition duration-200 justify-center backdrop-blur-sm text-[11px]">
                                        <CopyIcon className="w-3 h-3" /> Copy Title
                                    </button>
                                </div>

                                <div className="mt-3 pt-3 border-t border-white/5">
                                     <button onClick={handleCheckKeywords} className="flex items-center gap-1.5 text-gray-400 hover:text-brand-primary hover:bg-white/5 font-medium py-1 px-2 rounded-lg transition duration-200 w-full justify-center text-[10px] uppercase tracking-wide">
                                        <KeywordCheckerIcon className="w-3 h-3" /> Check Keywords on MyKeyworder
                                    </button>
                                </div>
                            </div>
                        )}
                        ...
                        {jsonPrompt && (
                             <div className="glass-panel rounded-2xl p-5 sm:p-6 animate-fade-in shadow-xl">
                                <div className="flex justify-between items-center mb-4">
                                     <h2 className="text-xl font-bold text-white">JSON Prompt</h2>
                                     <div className="flex items-center gap-2">
                                         <span className={`text-[10px] font-mono px-1.5 py-0.5 rounded bg-black/30 ${jsonCharCountColor}`}>{jsonCharCount} / 910</span>
                                         <button 
                                            onClick={() => setJsonPrompt(null)} 
                                            className="text-gray-400 hover:text-white hover:bg-white/10 rounded-lg p-1 transition-colors"
                                            title="Close JSON Prompt"
                                         >
                                            <CloseIcon />
                                         </button>
                                     </div>
                                ...
                                <div className="relative bg-black/50 rounded-xl border border-white/10 overflow-hidden">
                                    <div className="absolute top-0 left-0 w-full h-7 bg-white/5 border-b border-white/5 flex items-center px-3 gap-1.5">
                                        <div className="w-2.5 h-2.5 rounded-full bg-red-500/50"></div>
                                        <div className="w-2.5 h-2.5 rounded-full bg-yellow-500/50"></div>
                                        <div className="w-2.5 h-2.5 rounded-full bg-green-500/50"></div>
                                    </div>
                                    <pre className="text-xs p-4 pt-10 overflow-x-auto text-blue-100 whitespace-pre-wrap font-mono custom-scrollbar max-h-[400px]">
                                        <code>{jsonString}</code>
                                    </pre>
                                </div>

                                <div className="mt-5 flex flex-col sm:flex-row gap-2">
                                    <button onClick={() => handleModifyPrompt('color')} disabled={!!isModifying} className="flex items-center gap-1.5 bg-white/5 hover:bg-white/10 border border-white/10 text-gray-200 font-medium py-1 px-2 rounded-xl transition duration-200 w-full justify-center disabled:opacity-50 text-[11px]">
                                        {isModifying === 'color' ? <Spinner className="h-3 w-3" /> : <MagicWandIcon className="w-3 h-3 text-pink-400" />}
                                        <span className="bg-clip-text text-transparent bg-gradient-to-r from-pink-300 to-purple-300 font-bold">Remix Color</span>
                                    </button>
                                    <button onClick={() => handleModifyPrompt('style')} disabled={!!isModifying} className="flex items-center gap-1.5 bg-white/5 hover:bg-white/10 border border-white/10 text-gray-200 font-medium py-1 px-2 rounded-xl transition duration-200 w-full justify-center disabled:opacity-50 text-[11px]">
                                       {isModifying === 'style' ? <Spinner className="h-3 w-3" /> : <MagicWandIcon className="w-3 h-3 text-blue-400" />}
                                       <span className="bg-clip-text text-transparent bg-gradient-to-r from-blue-300 to-cyan-300 font-bold">Remix Style</span>
                                    </button>
                                    <button onClick={() => handleCopy(jsonString, 'prompt')} className="flex items-center gap-1.5 bg-brand-primary hover:bg-brand-secondary text-white font-bold py-1 px-2 rounded-xl transition duration-200 w-full justify-center shadow-lg shadow-brand-primary/20 text-[11px]">
                    ...
                    {showIframe && (
                        <div className="animate-fade-in lg:sticky lg:top-6 h-full">
                            <div className="glass-panel rounded-2xl p-1 w-full h-[75vh] shadow-2xl border border-white/20">
